<?php
	...
	function pushcookie($cookie_name, $cookie_value)
	{
		...
		if(!isset($_COOKIE[$cookie_name]))
		{
			...
			setcookie($cookie_name, serialize($new_value), time() + (86400 * 30), "/" );
		}
		else
		{
			...
			if(!isset($this_cookie[$cookie_value]))
			{
				...
				setcookie($cookie_name, serialize($this_cookie), time() + (86400 * 30), "/" );
			}
		}
	}
	...
	function pullcookie($cookie_name, $current_name = null)
	{
		...
		if(!isset($_COOKIE[$cookie_name]))
		{
			setcookie($cookie_name, serialize($pages_visited), time() + (86400 * 30), "/" );
		}
		else
		{
	...
	function dropcookie($cookie_name, $cookie_value)
	{
		...
		if(isset($_COOKIE[$cookie_name]))
		{
			...
			setcookie($cookie_name, serialize($new_cookie), time() + (86400 * 30), "/" );
		}
	}
